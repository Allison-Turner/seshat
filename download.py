...
def bash_wget__download(timestamp, loc, ipv, year, month, day, url, topo_choice, ext):
    ...
    if ipv==4:
        ...
        for file in properties.itdk_file_types:
            ...
            download_cmd = subprocess.Popen(["/usr/bin/wget", "-a", download_log, "-S", "-P", loc, url + year + "-" + month + "/" + topo_choice + file + ext ])
        ...
        download_cmd = subprocess.Popen(["/usr/bin/wget", "-a", download_log, "-S", "-P", loc, url + year + "-" + month + "/" + "itdk-run-" + year + month + day + "-dns-names.txt" + ext ])
        download_cmd.communicate()
        log_util.log_cmd_results(download_cmd, download_log)

        download_log.write("Downloading itdk-run-" + year + month + day + ".addrs" + ext + "\n")
        download_cmd = subprocess.Popen(["/usr/bin/wget", "-a", download_log, "-S", "-P", loc, url + year + "-" + month + "/" + "itdk-run-" + year + month + day + ".addrs" + ext ])
        download_cmd.communicate()
        log_util.log_cmd_results(download_cmd, download_log)
